...
> Last regenerated: 2026-04-23T23:28:02.893Z
...
Total TypeScript/TSX files: **514**
...
| File | Purpose | Exports |
|------|---------|---------|
| `appModeStore.ts` | appModeStore — Phase 14.R.3. | AppMode, AppModeState, useAppModeStore, APP_MODE_LABELS, +4 |
...
| `ModeAccentStripe.tsx` | ModeAccentStripe — thin workspace-accent strip along the top of the viewport, always visible. | ModeAccentStripe |
| `ModeTabs.tsx` | ModeTabs — Phase 14.R.3 (redesigned). | ModeTabs |
...
| `StatusBar.tsx` | StatusBar — bottom-of-screen bar for the plumbing workspace. Shows current draw mode, plane, diamete… | StatusBar |
...
| `ModeAccentStripe.spec.tsx` | ModeAccentStripe tests — peripheral-vision mode cue. | — |
| `ModeTabs.spec.tsx` | ModeTabs tests — redesigned workspace switcher. | — |
| `panelAccents.spec.tsx` | Inspector panel accents — regression tests for the workspace-accent borders added to panels that fac… | — |
...
| File | Purpose | Exports |
|------|---------|---------|
| `modeCursor.ts` | modeCursor — workspace-accent-tinted draw cursor. | PlumbingSubMode, RoofingSubMode, ModeCursorInput, shouldShowDrawCursor, +2 |
| `useCanvasCursor.ts` | useCanvasCursor — reactive cursor string for the 3D canvas. | useCanvasCursor |

### `src/ui/cursors/__tests__/`

| File | Purpose | Exports |
|------|---------|---------|
| `modeCursor.spec.ts` | modeCursor — unit tests for the pure cursor-resolution helpers. | — |
...
| `useAccentPulse.ts` | useAccentPulse — one-shot pulse scale when the workspace accent changes while the radial menu is ope… | useAccentPulse, __testables |
...
| `useAccentPulse.spec.tsx` | useAccentPulse — unit tests. | — |
...
### `src/ui/radial/wheels/__tests__/`

| File | Purpose | Exports |
|------|---------|---------|
| `DrawingWheel.spec.tsx` | DrawingWheel — workspace-accent integration tests. | — |
...
| `RoofingStatusBar.tsx` | RoofingStatusBar — peer to the plumbing `StatusBar`, mounted while `appMode === 'roofing'`. | RoofingStatusBar |
...
### `src/ui/roofing/__tests__/`

| File | Purpose | Exports |
|------|---------|---------|
| `RoofingStatusBar.spec.tsx` | RoofingStatusBar tests — domain-scoped status strip for the roofing workspace. | — |
...
| File | Purpose | Exports |
|------|---------|---------|
| `accentContrast.ts` | accentContrast — pick readable foreground text for any accent. | DARK_TEXT, LIGHT_TEXT, parseHexRgb, relativeLuminance, +2 |
| `InteractiveButton.tsx` | InteractiveButton — thin wrapper around a native `<button>` that applies the universal press + focus… | InteractiveButtonProps, InteractiveButton |
| `useInteractiveButton.ts` | useInteractiveButton — small state-machine hook for chrome buttons. | InteractiveButtonState, InteractiveButtonBindings, useInteractiveButton |

### `src/ui/shared/__tests__/`

| File | Purpose | Exports |
|------|---------|---------|
| `accentContrast.spec.ts` | accentContrast — unit tests. | — |
| `InteractiveButton.spec.tsx` | InteractiveButton — smoke tests for the shared button wrapper. | — |
| `useInteractiveButton.spec.tsx` | useInteractiveButton — unit + integration tests. | — |
...
_Generated by `tools/generate-reference.cjs` on 2026-04-23T23:28:03.014Z_
