...
describe('Scenario 2 — PVC → PEX mid-session switch', () => {
  ...
  it('diameter drops 2" → 0.75" on switch; new commit uses new settings', () => {
    ...
    usePlumbingDrawStore.getState().setDrawMaterial('pex');
    expect(usePlumbingDrawStore.getState().drawDiameter).toBe(0.75);

    // Draw + commit a PEX pipe
    const s2 = usePlumbingDrawStore.getState();
...
describe('Scenario 5 — Backspace mid-draw', () => {
  ...
  it('pops last point, keeps session alive', () => {
    ...
    s.addDrawPoint([5, 0, 0]);
    s.addDrawPoint([5, 0, 5]);
    expect(usePlumbingDrawStore.getState().drawPoints).toHaveLength(3);
    s.popDrawPoint();
    expect(usePlumbingDrawStore.getState().drawPoints).toHaveLength(2);
...
describe('Scenario 6 — Alt-click bypass', () => {
  ...
  it('addDrawPointRaw preserves odd angle through commit', () => {
    ...
    const last = usePlumbingDrawStore.getState().drawPoints[2]!;
...
describe('Scenario 7 — material switch thresholds', () => {
  ...
  it('PVC @ 2" → PEX → 0.75"', () => {
    usePlumbingDrawStore.setState({ drawMaterial: 'pvc_sch40', drawDiameter: 2 });
    usePlumbingDrawStore.getState().setDrawMaterial('pex');
    expect(usePlumbingDrawStore.getState().drawDiameter).toBe(0.75);
  });

  it('PEX @ 0.75" → PVC → 2"', () => {
    usePlumbingDrawStore.setState({ drawMaterial: 'pex', drawDiameter: 0.75 });
    usePlumbingDrawStore.getState().setDrawMaterial('pvc_sch40');
    expect(usePlumbingDrawStore.getState().drawDiameter).toBe(2);
  });

  it('PEX @ 0.75" → copper_type_l: no change (both supply)', () => {
    usePlumbingDrawStore.setState({ drawMaterial: 'pex', drawDiameter: 0.75 });
    usePlumbingDrawStore.getState().setDrawMaterial('copper_type_l');
  ...
  it('PVC @ 1.5" (boundary) → PEX → 0.75"', () => {
    usePlumbingDrawStore.setState({ drawMaterial: 'pvc_sch40', drawDiameter: 1.5 });
    usePlumbingDrawStore.getState().setDrawMaterial('pex');
